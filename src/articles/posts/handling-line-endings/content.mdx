...
1. We continue making changes, eventually we've made changs in 3 different files
1. [We submit the PR, and notice the diff is unreadable!](https://github.com/FormidableLabs/prism-react-renderer/pull/83/files/e6d4f1ec6039205cdae494ddbc1da3138e7a2a2b)
...
The `core.autocrlf` setting controls how git handles the line endings of files, this includes on checkout and on commit.
...
* **Windows**
Reccomends **true**, which gives you `CRLF` locally, but `LF` in source control
* **Unix / MacOS**
Reccomends **input**, which ensures that code is `LF` when checked in, but doesn't convert on checkout
...
I hope you've found something useful in this article; most of this information has come from me having to deal with line endings issues, but the ASCII roots informaation I read in [Charles Petzold](https://en.wikipedia.org/wiki/Charles_Petzold)'s excellent book [CODE](https://en.wikipedia.org/wiki/Code:_The_Hidden_Language_of_Computer_Hardware_and_Software).
